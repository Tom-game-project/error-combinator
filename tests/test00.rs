...
mod tests_n {
    ...
    fn n_works00() {
        let check_list  = [
            ("hello abc world",None,),
            ("abc world",      Some(vec![ValidateErr::CheckStartsWithHelloErr]),),
            ("hello world",    Some(vec![ValidateErr::CheckIncludesAbcErr]),),
            ("hello abc",      Some(vec![ValidateErr::CheckEndsWithWorldErr]),),
            ("hello--",        Some(vec![ValidateErr::CheckEndsWithWorldErr]),),
            ("abc----",        Some(vec![ValidateErr::CheckStartsWithHelloErr]),),
            ("world--",        Some(vec![ValidateErr::CheckStartsWithHelloErr]),),
        ];

        for (s, ans) in check_list {
            let checker =
    ...
    fn n_works01() {
        let check_list  = [
            ...
            ("hello world",     Some(vec![ValidateErr::CheckIncludesAbcErr]),),
            ("hello abc",       Some(vec![ValidateErr::CheckEndsWithWorldErr]),),
            ("hello--",         Some(vec![ValidateErr::CheckEndsWithWorldErr, ValidateErr::CheckIncludesAbcErr]),),
            ("abc----",         Some(vec![ValidateErr::CheckStartsWithHelloErr, ValidateErr::CheckEndsWithWorldErr]),),
            ("world--",         Some(vec![ValidateErr::CheckStartsWithHelloErr, ValidateErr::CheckEndsWithWorldErr, ValidateErr::CheckIncludesAbcErr]),),
            ("a",               Some(vec![ValidateErr::CheckStartsWithHelloErr, ValidateErr::CheckMin6Err, ValidateErr::CheckEndsWithWorldErr, ValidateErr::CheckIncludesAbcErr]),),
        ];

        for (s, ans) in check_list {
            let checker =
                (check_starts_with_hello
                .or::<_, VecCombine<ValidateErr>>(check_min6))
                .or::<_, VecCombine<ValidateErr>>(check_ends_with_world)
                .or::<_, VecCombine<ValidateErr>>(check_includes_abc);
